...
def create_hostapd_conf2_4_dual(iface, ssid, channel):
    print("[*] Creating hostapd.conf...")
    config_content = f"""
interface={iface}
driver=nl80211
...
"""
    try:
        with open("hostapd_dual.conf", "w") as f:
            f.write(config_content.strip())
        print("[+] hostapd.conf created successfully.")
    except Exception as e:
        print(f"[-] Failed to create hostapd.conf: {e}")
...
def create_hostapd_conf5_dual(iface, ssid, channel):
    print("[*] Creating hostapd.conf...")
    config_content = f"""
interface={iface}
driver=nl80211
...
def main():
    ...
    parser.add_argument("--Dual", dest="dual", action="store_true", help="Choose Your Captive Portal")
    ...
    if script_args.dual:
        print("[>] Skipping on manage service")
    else:
    ...
    time.sleep(1)
    if script_args.dual:
        print("[>] Skipping on configure interface ")
    else:
    ...
    if script_args.band == '2.4':
        if script_args.dual:
            create_hostapd_conf2_4_dual(script_args.iface, script_args.ssid, script_args.channel)
        else:
            create_hostapd_conf2_4(script_args.iface, script_args.ssid, script_args.channel)

    if script_args.band == '5':
        if script_args.dual:
            create_hostapd_conf5_dual(script_args.iface, script_args.ssid, script_args.channel)
        else:
            create_hostapd_conf5(script_args.iface, script_args.ssid, script_args.channel)

    if script_args.dual:
        print("[>] Skipping on starting apache2 ")
    else:
        setup_apache()
    ...
    if script_args.dual:
        print("[>] Skipping on Iptables configuration ")
    else:
