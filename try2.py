...
def start_attack_dual(channel1: str, channel2: str):
    ...
    try:
        print("[*] Starting dnsmasq...")
        dnsmasq_proc = subprocess.Popen(['dnsmasq', '-C', 'dnsmasq_dual.conf', '-d'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        
        print("[*] Starting hostapd...")
        hostapd_proc = subprocess.Popen(['hostapd', 'hostapd_24.conf'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        hostapd_proc2 = subprocess.Popen(['hostapd', 'hostapd_5.conf'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        if hostapd_proc2.poll() is not None:
            stdout, stderr = hostapd_proc2.communicate()
            print("[-] hostapd failed to start. Error output:")
            print(stderr.decode().strip())
            raise Exception("hostapd failed.")  
